...
function checkTrigger() {
    ...
    var matching = script.gestureController.isMatchingCustomGesture(
        script.customGesture, script.threshold * THRESHOLD_MULTIPLIER
    );
    
    if (matching && !triggerStarted) {
        // Only fire ONCE when gesture first detected
        global.behaviorSystem.sendCustomTrigger(script.customResponseStart);
        triggerStarted = true;
        triggeredTime = getTime();
        printDebug(" started", script.debug);
    } else if (!matching && triggerStarted && getTime() - triggeredTime > COOL_DOWN) {
        // Fire end only when gesture stops
